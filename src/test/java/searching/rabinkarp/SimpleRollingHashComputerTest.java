...
class SimpleRollingHashComputerTest {

    private static final RollingHashComputer HASH_COMPUTER = new SimpleRollingHashComputer(1000);

    @Test
    void hash_is_simply_sum_of_contained_chars() {
        long hashValue = HASH_COMPUTER.computeFullHash("abcde");
        assertEquals('a' + 'b' + 'c' + 'd' + 'e', hashValue);
    }

    @Test
    void rollingHash_is_simply_sum_of_contained_chars() {
        long hashValue = HASH_COMPUTER.computeFullHash("abcde");
        long afterOneRoll = HASH_COMPUTER.rollToRight(hashValue, "abcdef".toCharArray(), 0, 5);
        assertEquals('b' + 'c' + 'd' + 'e' + 'f', afterOneRoll);
    }
